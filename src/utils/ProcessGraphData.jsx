...
const processTransactionData = (data, threshold, centralNodeAddress, ) => {
  const formatAddress = (address) => 
    `${address.slice(0, 5)}...${address.slice(-5)}`;
  const processedNodes = [];
  const processedEdges = [];
  ...
  data.forEach((item) => {
    const valueInBTC = item.value / 100000000; // Assuming value is in satoshis
    if (valueInBTC < threshold) return;
    ...
    if (!processedEdges.some((edge) => edge.id === edgeId)) {
      const edge = {
        ...
        target: toNodeID,
        targetHandle: `${toNodeID}-t`,
        label: `${valueInBTC.toFixed(4)} BTC`,
        animated: true,
        style: { stroke: edgeColor, strokeWidth: edgeWidth, color: "grey" },
      };
      processedEdges.push(edge);
